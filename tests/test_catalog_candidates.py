...
Ported to multi-tenancy: authed requests; frequency ranking is per-household
(the ordering must never reveal other households' buying), so the fixture
events carry this module's household_id."""
import json
import uuid
...
TOKEN, HH, _UID = register_household(client, "cand")
H = auth_hdr(TOKEN)
HID = HH["id"]
...
def test_api_catalog_shape_and_frequency_order():
    seed_catalog.seed(appmod.conn)
    # buy 食パン twice IN THIS HOUSEHOLD → it must outrank never-bought seeds
    ...
    for ts in ("2026-07-01T00:00:00+00:00", "2026-07-02T00:00:00+00:00"):
        appmod.conn.execute(
            ...
            (HID, cid, ts))
    appmod.conn.commit()
    cat = client.get("/api/catalog", headers=H).json()["catalog"]
    assert len(cat) >= len(seed_catalog.SEED)
    assert cat[0]["name"] == "食パン" and cat[0]["category"] == "bakery"
    assert "bread" in cat[0]["aliases"]
    # another household sees the shared seed corpus, but frequency ranking is
    # scoped: with zero buys of its own, its ordering is purely alphabetical
    tok_b, _, _ = register_household(client, "candB")
    cat_b = client.get("/api/catalog", headers=auth_hdr(tok_b)).json()["catalog"]
    names_b = [c["name"] for c in cat_b]
    assert "食パン" in names_b                       # corpus is shared
    assert names_b == sorted(names_b)                # no cross-tenant buys boost
...
def test_seeded_rows_have_categories_in_state():
    ...
    client.post("/api/op", json={"op_id": str(uuid.uuid4()), "actor": "t",
                ...
                headers=H)
    items = {i["name"]: i for i in client.get("/api/state", headers=H).json()["items"]}
