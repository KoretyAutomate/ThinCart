/*
 * PlantCart — API base shim for the native (Capacitor) build.
 * ============================================================
 *
 * WHY THIS EXISTS
 * ---------------
 ...
 * so "same origin" == "the PlantCart server". Inside a store-shipped native
 ...
 * call at a configured REMOTE backend (the hosted PlantCart server).
 *
 * This shim publishes a single global, `window.PLANTCART_API_BASE`, that the
 ...
 *
 * On the WEB build this file is absent (or PLANTCART_API_BASE is left unset),
 ...
 *
 *     const API_BASE = (window.PLANTCART_API_BASE || '').replace(/\/$/, '');
 ...
 * the WebSocket constructor uses `wsUrl()`. With PLANTCART_API_BASE unset the
